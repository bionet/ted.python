...
def gen_test_signal(dur,dt,fmax,np=None,nc=3):
    """Return a signal dur seconds long uniformly sampled at intervals
    dt. The generated signal''s frequency components are no higher than
    fmax Hz. If np != None, Gaussian white noise with power np (in dB) is
    added to the generated signal before the latter is filtered and
    returned."""
